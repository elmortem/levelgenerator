...
{
	public class VerticesProjection
    {
        private Mesh _mesh;
        private Vector3 _offset;
        private MeshSurfaceNormalMode _normalMode;

        public VerticesProjection(Mesh mesh, Vector3 offset, MeshSurfaceNormalMode normalMode)
        {
            _mesh = mesh;
            _offset = offset;
        ...
        public void GetSurfacePoints(List<PointData> points, int count)
        {
            ...
            for (int i = 0; i < vertices.Length; i += step)
            {
                var point = new PointData
                {
                    ...
                    Normal = NormalUtility.GetNormal(_normalMode, vertices[i], _offset, normals[i]),
                    Scale = Vector3.one
                };
                points.Add(point);
            }
            ...
            while (points.Count < count && usedIndices.Count < vertices.Length)
            {
                ...
                if (!usedIndices.Contains(index))
                {
                    var point = new PointData
                    {
                        ...
                        Normal = NormalUtility.GetNormal(_normalMode, vertices[index], _offset, normals[index]),
                        Scale = Vector3.one
                    };
                    points.Add(point);
