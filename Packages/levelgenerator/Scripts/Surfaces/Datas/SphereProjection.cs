...
namespace LevelGenerator.Surfaces.Datas
{
	public class SphereProjection
	{
		...
		public SphereProjection(Mesh mesh, Vector3 offset, MeshSurfaceNormalMode normalMode)
		{
			_mesh = mesh;
			_offset = offset;
			...
			foreach (var spherePoint in spherePoints)
			{
				...
				if (closestDistance < float.MaxValue)
				{
					var point = new PointData
					{
						...
						Normal = NormalUtility.GetNormal(_normalMode, closestIntersection, _offset, (closestIntersection - centerOfMass).normalized),
						Scale = Vector3.one
					};
					points.Add(point);
				}
				
				var sPoint = new PointData
				{
					Position = spherePoint + _offset,
					Normal = NormalUtility.GetNormal(_normalMode, spherePoint, _offset, (spherePoint - centerOfMass).normalized),
					Scale = Vector3.one
				};
				points.Add(sPoint);
			}
			...
			foreach (var point in spherePoints)
			{
				if (IsPointInsideMesh(point))
				{
					points.Add(new PointData
					{
						...
						Normal = NormalUtility.GetNormal(_normalMode, point, _offset, Vector3.zero),
						Scale = Vector3.one
					});
				}
				...
				{
					Position = point + _offset,
					Normal = NormalUtility.GetNormal(_normalMode, point, _offset, Vector3.up),
					Scale = Vector3.one
				};
				points.Add(sPoint);
			}
